...
//return string filled with file's content
...
std::string readFileIntoStringBin(const std::string& path) {
...
int main(int argc, char*argv[]) {
    ...
    std::string zeros = "00000000";
    if(argv[1][0] == 'c') {
       fileContent = readFileIntoString(fileName);
       std::ofstream fileCompressed(fileName+".lzw", std::ios::binary);
        ...
        */
        ...
        std::vector<int> hold;
        {
            for(int i = 0; i<s.size()-4; i+=12) {
                auto temp = s.substr(i, 12);
                hold.push_back(binaryString2Int(temp));
            }
        }
        auto decompressed = decompress(hold.begin(), hold.end());
        std::string saveFile = fileName;
        saveFile.erase(saveFile.length()-4, 4);
         auto x = saveFile.find(".");
         if(x != std::string::npos) {
            saveFile.insert(x, "2");
         }
         else {
            saveFile.append("2"); //for file without any extension
         }
         std::ofstream writeFile(saveFile);
